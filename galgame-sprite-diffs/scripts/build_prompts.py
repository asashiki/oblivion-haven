...
- First repaint the complete original open-eye construction as clean local skin: remove both irises, sclerae, lower lashes, the full-open upper-lash contours, and every antialiased gray/black echo.
- Then draw exactly one intentional closed-eyelid curve per eye in the original eye position.
- Leave no eyelid crease, eyelid fold, highlight line, shadow line, second arc, or faint duplicate stroke above or around the new closed lid. There must be exactly one intentional lid contour per eye. Preserve the eyebrows; they are not part of the eye replacement.
"""
    elif state == "eyes_half":
        eye_cleanup_rule = """
Eye-replacement rule:
- First remove the complete original full-open eye construction, including its upper/lower lash outlines and antialiased gray/black echoes; then redraw the reduced half-closed aperture cleanly.
...
- Preserve only the iris/sclera portion naturally visible through the new smaller aperture and preserve the eyebrows.
"""
...
- Do not recenter, rescale, expand, rotate, translate, or redraw the head or face. Keep all visible hair, nose, cheeks, ears, brows, and crop-edge pixels fixed.
