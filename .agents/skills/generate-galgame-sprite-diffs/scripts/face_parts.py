...
def command_mask(args: argparse.Namespace) -> None:
    ...
    brow_ellipses = args.brow_ellipse or []
    allow = build_allow_mask(base.size, [*args.ellipse, *brow_ellipses], args.feather)
    ...
    brow_mask = (
        np.asarray(build_allow_mask(base.size, brow_ellipses, 0), dtype=np.uint8)
        if brow_ellipses
        else np.zeros((base.height, base.width), dtype=np.uint8)
    )
    weight = (allow_array.astype(np.float32) / 255.0 * 0.42)[..., None]
    tint = np.zeros_like(base_array[..., :3], dtype=np.float32)
    tint[..., 0] = 255
    tint[..., 1] = 48
    tint[..., 2] = 96
    brow_pixels = brow_mask > 0
    tint[brow_pixels] = (255, 174, 32)
    blended = (
    ...
    write_json(
        args.json,
        {
            ...
            "eye_ellipses_normalized": [list(value) for value in args.ellipse],
            "brow_ellipses_normalized": [list(value) for value in brow_ellipses],
            "region_contract": "eyes-plus-optional-dynamic-brows",
...
def command_compose(args: argparse.Namespace) -> None:
    ...
    # Candidate skin/hair fill inside a legal mask can still create a visible
    # color block. Measure bright surface pixels separately from dark eye/mouth
    # ink so an oversized legal patch cannot pass solely because it stayed
    # inside the mask.
    base_rgb = base_array[..., :3]
    frame_rgb = frame_array[..., :3]
    base_luma = np.mean(base_rgb.astype(np.float32), axis=2)
    frame_luma = np.mean(frame_rgb.astype(np.float32), axis=2)
    surface = (
        inside
        & (base_array[..., 3] > 127)
        & (frame_array[..., 3] > 127)
        & (base_luma >= 110.0)
        & (frame_luma >= 110.0)
        & (np.min(base_rgb, axis=2) >= 36)
        & (np.min(frame_rgb, axis=2) >= 36)
    )
    surface_delta = np.mean(delta, axis=2)[surface]
    surface_median_delta = float(np.median(surface_delta)) if surface_delta.size else 0.0
    surface_p95_delta = float(np.percentile(surface_delta, 95.0)) if surface_delta.size else 0.0
    ...
    if surface_median_delta > args.max_surface_median_delta:
        failures.append("许可区域内肤色/浅色表面整体偏色")
    if surface_p95_delta > args.max_surface_p95_delta:
        failures.append("许可区域内存在明显肤色/浅色块或纹理断层")
    report = {
        ...
        "surface_sample_pixels": int(surface_delta.size),
        "surface_rgb_median_delta": round(surface_median_delta, 6),
        "surface_rgb_p95_delta": round(surface_p95_delta, 6),
        "surface_color_thresholds": {
            "median": args.max_surface_median_delta,
            "p95": args.max_surface_p95_delta,
        },
        "base_sha256": sha256(args.base),
...
def command_eye_review(args: argparse.Namespace) -> None:
    ...
    difference = np.max(
        np.abs(np.asarray(base, dtype=np.int16) - np.asarray(frame, dtype=np.int16)),
        axis=2,
    ).astype(np.uint8)
    heat = np.zeros((base.height, base.width, 3), dtype=np.uint8)
    heat[..., 0] = difference
    heat[..., 1] = np.where(
        difference > 0,
        np.minimum(difference.astype(np.uint16) * 2, 255),
        0,
    ).astype(np.uint8)
    unchanged_dark = (
        (np.asarray(allow, dtype=np.uint8) > 127)
        & (np.mean(np.asarray(base, dtype=np.uint8)[..., :3], axis=2) < 80)
        & (difference == 0)
    )
    crops = [
        ...
        ("base vs final change map", Image.fromarray(heat, mode="RGB").crop(crop_box)),
    ]
    ...
    write_json(
        args.json,
        {
            ...
            "crop_bbox": list(crop_box),
            "scale": args.scale,
            "unchanged_dark_pixels_inside_solid_mask": int(np.count_nonzero(unchanged_dark)),
            ...
            "required_visual_checks": [
                ...
                "the entire eye-and-brow context contains no remote black block, orphan lash tip, or disconnected old brow fragment",
                "eyebrows move subtly and coherently with the blink while preserving the expression; they do not teleport or split into old/new copies",
                "skin, eyelid fill, and nearby flat-color blocks match the approved mother in hue, value, texture, and antialiasing",
            ],
        },
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    mask.add_argument(
        ...
        action="append",
        default=[],
        help="可选眉毛活动区 cx,cy,rx,ry；眨眼时通常左右各一条",
    )
    ...
    compose.add_argument("--max-surface-median-delta", type=float, default=10.0)
    compose.add_argument("--max-surface-p95-delta", type=float, default=28.0)
    ...
    eye_review.add_argument("--padding", type=int, default=48)
...
def main() -> None:
    ...
    if hasattr(args, "max_surface_median_delta") and args.max_surface_median_delta < 0:
        raise SystemExit("--max-surface-median-delta 不可小于 0")
    if hasattr(args, "max_surface_p95_delta") and args.max_surface_p95_delta < 0:
        raise SystemExit("--max-surface-p95-delta 不可小于 0")
