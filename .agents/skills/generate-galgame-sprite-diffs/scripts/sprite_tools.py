#!/usr/bin/env python3
"""Portable alpha routing, guarded chroma fallback, normalization, and sprite QA."""
...
def command_alpha_route(args: argparse.Namespace) -> None:
    """Choose the input path from the file's real alpha, never its preview."""
    ...
    try:
        ...
        raise SystemExit(f"图片无法完整读取 {args.input}: {exc}") from exc

    alpha = rgba[..., 3]
    has_alpha_channel = "A" in bands or (mode == "P" and "transparency" in image_info)
    ...
    if has_alpha_channel and alpha_min < 255 and visible_pixels:
        route = "use-authoritative-rgba"
        action = "skip-generation-and-cutout"
    elif visible_pixels == 0:
        route = "reject-empty-alpha"
        action = "stop"
    else:
        route = "opaque-reference-to-transparent-img2img"
        action = "one-fidelity-locked-transparent-edit"

    report = {
        "operation": "alpha-input-route",
        ...
        "route": route,
        "action": action,
        "chroma_fallback_allowed": bool(args.allow_chroma_fallback),
        "policy": (
            "Never cut out or re-matte a valid RGBA source. For opaque input, request one "
            "true-transparent image-to-image conversion and validate the returned alpha. "
            "A chroma cutout is opt-in fallback only."
        ),
    }
    write_json(args.json, report)
    if route == "reject-empty-alpha":
        raise SystemExit(2)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    alpha_route = subparsers.add_parser(
        "alpha-route", help="按文件真实 alpha 决定直通或透明图生图路径"
    )
    alpha_route.add_argument("input", type=Path)
    alpha_route.add_argument(
        "--allow-chroma-fallback",
        action="store_true",
        help="仅在用户明确允许时记录可使用色键后备",
    )
    alpha_route.add_argument("--json", type=Path, help="保存 JSON 报告")
    alpha_route.set_defaults(func=command_alpha_route)
