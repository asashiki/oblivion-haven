...
<head>
  ...
  <style>html,body{width:100%;height:100%;margin:0;background:#05060b;color:#fff;overflow:hidden}#ebg{position:fixed;inset:-8%;background:#05060b;filter:blur(36px)}#ebgOverlay{width:100%;height:100%}#root{position:absolute;width:2560px;height:1440px;transform-origin:top left;overflow:hidden}[class*="_Title_main_"],[class*="_main_rdjpk_"],[class*="_trans_1oupq_"]{display:none!important}#galblog-language-gate{position:fixed;inset:0;z-index:2147483647;display:grid;place-items:center;background:#05060b;color:#fff;font-family:system-ui,sans-serif}#galblog-language-gate[hidden]{display:none}#galblog-language-gate>div{display:flex;gap:8px;padding:8px;border:1px solid #ffffff1f;border-radius:16px;background:#ffffff0a;box-shadow:0 20px 80px #0008}#galblog-language-gate button{min-width:96px;border:0;border-radius:11px;padding:12px 16px;background:transparent;color:#d9dce7;font:600 14px system-ui;cursor:pointer}#galblog-language-gate button:first-child,#galblog-language-gate button:hover{background:#fff;color:#171922}#galblog-engine-status{position:absolute;left:50%;top:50%;z-index:101;width:18px;height:18px;border:2px solid #ffffff26;border-top-color:#fff;border-radius:50%;transform:translate(-50%,-50%);animation:galblog-spin .8s linear infinite;font-size:0}@keyframes galblog-spin{to{transform:translate(-50%,-50%) rotate(360deg)}}</style>
...
<body>
  ...
  <script>
    (()=>{const root=document.getElementById("root"),status=document.getElementById("galblog-engine-status"),gate=document.getElementById("galblog-language-gate");window.__GAL_BLOG_LANGUAGE_READY__=(()=>{const saved=localStorage.getItem("lang");if(["0","1","2"].includes(saved||""))return Promise.resolve(saved);gate.hidden=false;return new Promise(resolve=>gate.querySelectorAll("button").forEach(button=>button.addEventListener("click",()=>{const language=button.dataset.lang||"2";localStorage.setItem("lang",language);gate.remove();resolve(language);},{once:true})));})();const click=target=>target?.dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true}));const waitFor=(fn,timeout=8000)=>new Promise((resolve,reject)=>{const started=Date.now(),check=()=>{const target=fn();if(target)return resolve(target);if(Date.now()-started>=timeout)return reject(new Error("WebGAL start control was not found"));setTimeout(check,40);};check();});window.__GAL_BLOG_ENGINE_RENDERED__.then(async()=>{click(await waitFor(()=>document.querySelector(".title__enter-game-target")));click(await waitFor(()=>Array.from(document.querySelectorAll("div")).find(element=>String(element.className).includes("_Title_button_")&&String(element.parentElement?.className).includes("_Title_buttonList_"))));}).catch(error=>{if(status)status.textContent=error?.message||String(error);});const resize=()=>{const scale=Math.min(innerWidth/2560,innerHeight/1440),left=(innerWidth-2560*scale)/2,top=(innerHeight-1440*scale)/2;root.style.transform="translate("+left+"px,"+top+"px) scale("+scale+")";};resize();addEventListener("resize",resize);})();
  </script>
  <script type="module">
